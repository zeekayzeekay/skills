...
Invoke `delivery-mode-engineering` through the harness's skill tool, or read its `SKILL.md` if no skill tool exists. Use its shared delivery context and Review dispositions against these governing sources. Reuse the selected mode and accepted limitations; an inherited routine fix does not restart calibration. A review-only request does not install a missing contract or change repository rules.
...
For substantial or consequential changes, use separate clean-context subagents for the three axes when available. Supply only the task boundary, pinned source/snapshot, shared delivery context, contract, standards, relevant raw fixtures and safe test commands. Give every reviewer access to the same delivery-mode definitions, including an external reviewer reached through a handoff. **Do not supply the implementer's completion narrative, suspected defects, proposed fixes or another reviewer's conclusions.** Explicitly forbid recursive delegation and source edits. Do not inherit the authoring conversation.
...
Report under **Standards**, **Spec** and **Assurance**, retaining their distinct meanings. Cross-reference overlapping findings rather than counting one defect twice. Include severity, the separate current-delivery disposition from `delivery-mode-engineering`, precise location, governing current claim, failing or unsupported state, observable consequence and the smallest useful correction or missing check. If the host fixes the report schema, put the disposition and rationale in the finding body. A hard standards breach needs a documented rule; an Assurance gap is not automatically a proven behavioral defect. Use the shared Review rules to decide whether a missing test prevents readiness or is a nonblocking improvement.

Close with counts and worst issue per axis, current blockers separately from other dispositions, the exact reviewed state, commands/results, skipped paths and remaining uncertainty. No findings means none found in this bounded scope, not proof of correctness. Review materially changed paths after authorized fixes. Apply the shared stopping rule when repeated findings add no new evidence or fixes oscillate.
